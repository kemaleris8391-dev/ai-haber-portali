...
1. Python `fetcher.py` taramaya başlamadan önce doğrudan `web-portal/src/content/blog` klasöründeki son **30 saat** içinde yayınlanmış olan aktif haber dosyalarını okur ve sitemizde yakın zamanda yayınlanmış olan haber başlıklarını canlı olarak belleğe yükler (devasa klasörün tamamını okuyarak belleği şişirmez).
...
3. Yeni haber başlığının, sitemizdeki son 30 saatte yayınlanmış olan başlıklarla ortak kelime kesişim oranını (kelime ve karakter düzeyinde) hesaplar.
4. **Kelime Benzerlik Oranı >= %30 veya Karakter Benzerlik Oranı >= %38** ise, sistem bu haberi *"Aynı konu zaten sitemizde yayınlanmış!"* diyerek **akıllıca es geçer ve eler.**
