...
2. Haberlerin odağı sadece PLC programlama ve otomasyonu, PC donanımları ve bilgisayar teknolojileri, endüstriyel makineler ve bunların bakım-tamirleri, dijital/bilgisayar oyun dünyası ile yapay zeka ve makine öğrenimi olmalıdır.
3. Siyaset, politika, magazin, aşk, genel finans/kripto para ve alakasız konular KESİNLİKLE sisteme alınmamalı, portalımızın teknoloji, otomasyon, oyun ve yapay zeka odağı dışı ilan edilmelidir.
4. EĞER GİRDİ HABERİ BU BELİRTİLEN SINIRLARIN (PLC, PC, Endüstriyel Makineler, Oyun, Yapay Zeka) DIŞINDAYSA, kesinlikle makale yazma ve sadece aşağıdaki hata formatında JSON dön:
{
  "error": "Bu konu/haber portalımızın odak alanı (PLC, PC, Endüstriyel Makineler, Oyun, Yapay Zeka) dışındadır."
}
...
1. YAYİN POLİTİKASI UYGUNLUK KONTROLÜ (is_compliant):
   ...
   - Yayın Politikası Odak Alanları: PLC otomasyonu (plc), kişisel bilgisayarlar ve donanımlar (pc), endüstriyel makineler (endustriyel-makinalar), oyun dünyası (oyun) ile yapay zeka (yapay-zeka) ile ilgili olmalıdır.
...
5. Haber kategorisini konuya göre tam olarak şu beşinden biri olarak belirle: "plc", "pc", "endustriyel-makinalar", "oyun" veya "yapay-zeka". Başka bir kategori adı kesinlikle kullanma.
6. Haberle ilgili 5 adet Türkçe etiket (keywords) belirle.
7. Pexels görsel arama motoru için haberin ana konusunu, markasını ve modelini içeren İngilizce 2-3 kelimelik net ve nokta atışı bir görsel arama sorgusu (pexels_query) yaz. Örnek: "playstation 5 console" (sadece "playstation" yazma), "intel arc gpu" (sadece "gpu" yazma), "quantum computing chip" (sadece "quantum" yazma), "volkswagen ID electric car" (sadece "car" yazma).
...
"""
    ...
    categories_ref.set({
        "list": ["plc", "pc", "endustriyel-makinalar", "oyun", "yapay-zeka"]
    })
